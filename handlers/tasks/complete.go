...
func CompleteTaskHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if task.ParentTaskID != nil {
			...
			tx.Model(&models.Task{}).Where("parent_task_id = ? AND status = ?", task.ParentTaskID, models.StatusCompleted).Count(&completedCount)

			// Если все подзадачи завершены, обновляем статус родительской задачи
