				...
				"Tasks": {
					...
					},
					{
						...
						Response: map[string]interface{}{
							...
							"_note":  "Если все подзадачи завершены, родительская задача переводится в status = submitted",
						},
						Errors: []ErrorInfo{
		...
		info.Endpoints["Business Logic"] = []Endpoint{
			{
				...
				Response: map[string]interface{}{
					"rules": []string{
						...
						"3. При завершении всех подзадач родительская задача автоматически переводится в статус 'submitted'",
