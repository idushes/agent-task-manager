		...
		info.Endpoints["Business Logic"] = []Endpoint{
			{
				...
				Response: map[string]interface{}{
					"rules": []string{
						...
						"11. При переходе задачи в статус 'submitted' автоматически отправляется уведомление в Redis очередь 'task_notifications' с ID задачи и именем исполнителя",
						"12. Автоматическая очистка задач с истекшим DeleteAt запускается каждый час (настраивается через CLEANUP_INTERVAL)",
					},
				},
			},
		}
		...
		info.Endpoints["Security"] = []Endpoint{
			{
				...
				Response: map[string]interface{}{
					...
					"environment_variables": map[string]string{
						...
						"REDIS_URL":         "URL для подключения к Redis (обязательный для работы уведомлений)",
					},
				},
			},
		}

		// Добавляем информацию о Redis интеграции
		info.Endpoints["Redis Integration"] = []Endpoint{
			{
				Method:      "INFO",
				Path:        "",
				Description: "Интеграция с Redis для уведомлений",
				Auth:        false,
				Response: map[string]interface{}{
					"queue_name": "task_notifications",
					"notification_format": map[string]string{
						"task_id":   "UUID задачи",
						"assignee":  "Имя пользователя, которому назначена задача",
						"timestamp": "Временная метка отправки уведомления",
					},
					"triggers": []string{
						"1. При создании новой задачи со статусом 'submitted'",
						"2. При завершении всех подзадач и переводе родительской задачи обратно в 'submitted'",
						"3. При отмене всех подзадач и переводе родительской задачи обратно в 'submitted'",
					},
					"requirements": []string{
						"Redis должен быть доступен при старте приложения",
						"Приложение автоматически создает очередь при старте, если она не существует",
						"При невозможности подключения к Redis приложение завершается с ошибкой",
					},
				},
			},
		}
