		...
		info.Endpoints["Business Logic"] = []Endpoint{
			{
				...
				Response: map[string]interface{}{
					"rules": []string{
						...
						"4. При завершении задачи все её активные подзадачи (submitted, working, waiting) рекурсивно отменяются",
						"5. Только assignee может взять задачу в работу и завершить её",
						"6. Задачи автоматически удаляются через 3 месяца (можно изменить при создании)",
						"7. Каждая задача имеет root_task_id для отслеживания иерархии",
					},
				},
			},
		}
